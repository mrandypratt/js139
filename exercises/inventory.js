...
let ItemManager = (() => {
  function _fiveCharNoSpaces(string) {
    let matches = string.match(/[a-z]/ig);
    return matches.length >= 5;
  }
  
  function _validName(name) {
    return _fiveCharNoSpaces(name);
  }
  
  function _validCategory(category) {
    return (_fiveCharNoSpaces(category) && 
      (category.match(/[\s]/) === null));
  }
  
  function _validQuantity(quantity) {
    return (!isNaN(quantity) && quantity >= 0);
  }
  
  function _getSKULetters(string, numChars) {
    let SKULetters = "";
  ...
  return {
    ...
    create(name, category, quantity) {
      ...
      if (!_validCategory(category)) return false;
      if (!_validQuantity(quantity)) return false;
      let SKU = _getSKULetters(name, 3) + _getSKULetters(category, 2);
